...
class SEIGrowth(BaseModel):
    ...
    def get_coupled_variables(self, variables):
        ...
        eta_SEI = delta_phi - phase_param.U_sei - j * L_sei * R_sei*0
        ...
        if SEI_option.startswith("reaction limited"):
            ...
            j_sei = -param.F * phase_param.k_sei * phase_param.c_sol * pybamm.exp(-alpha_SEI * F_RT * delta_phi)
        elif SEI_option == "electron-migration limited":
            # Scott Marquis thesis (eq. 5.94)
            j_sei = (eta_SEI < 0) * phase_param.kappa_inner * eta_SEI / L_sei    
        elif SEI_option == "Kolzenberg2020":
            eta_bar = F_RT * (delta_phi)
            j0_sei = param.F * phase_param.k_sei * phase_param.c_sol
            L_diff = (phase_param.D_li * phase_param.c_li_0 * param.F / j0_sei) \
            ...
            j_sei =  - LL_k0 * j0_sei * pybamm.exp(-alpha_SEI * eta_bar)
            ...
            beta = 1/2.0e-9
            j_sei = - param.F * phase_param.k_sei * phase_param.c_sol \
                * pybamm.exp(-alpha_SEI * F_RT * delta_phi) \
                * pybamm.exp(-beta * L_sei)
            ...
            eta_bar = F_RT * (delta_phi)
            # j_sei =  (j <=0) *phase_param.D_li * phase_param.c_li_0 * param.F* F_RT  * j /(2*phase_param.kappa_Li_ion )* pybamm.exp(- eta_bar)
            ...
            L_over_D = L_sei / phase_param.D_sol
            c_0 = phase_param.c_sol
            j_sei = -param.F * c_0 * k_exp / (1 + L_over_D * k_exp)
            c_ec = c_0 / (1 + L_over_D * k_exp)
            
            
            # RR = 1/( param.F * phase_param.k_sei * c_0)
            # L_sei_0 = phase_param.L_inner_0 + phase_param.L_outer_0
            # DD = L_sei_0/(phase_param.D_sol * c_0 * param.F)
            
    
        
            # L_sei_hat = L_sei / L_sei_0
            
            
            # j_sei = -1/(RR *pybamm.exp(alpha_SEI * F_RT * eta_SEI) \
            #     + DD * L_sei_hat) 
